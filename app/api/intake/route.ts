...
import { isRateLimited, isDemoMode } from "@/lib/security";
import { ensureApplicationUser, getAuthenticatedUser } from "@/lib/auth";
...
export async function POST(request: Request) {
  try {
    ...
    if (!isDemoMode()) {
      ...
      if (!user) return NextResponse.json({ error: "Authentication required for persisted intake." }, { status: 401 });
