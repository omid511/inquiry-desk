...
class SupabaseInquiryStore implements InquiryStore {
  ...
  private async syncRelated(lead: InquiryLead) {
    ...
    const { error: messageError } = await this.client.from("inquiry_messages").upsert(lead.messages.map((message) => ({ id: message.id, lead_id: lead.id, ...message, created_at: message.createdAt, created_by: message.createdBy })), { onConflict: "id", ignoreDuplicates: true }); if (messageError) throw messageError;
